...
module Repr = struct
  ...
  let create ~t ~univ_constr = { t; univ_constr }
...
module V2 = struct
  ...
  let of_prev prev =
    let { Prev.uuid; ml_bundles; repr } = prev in
    let repr =
      match repr with
      | Some repr -> repr
      | None -> Repr.side_effect
    in
    { uuid; ml_bundles; repr }
  ;;
end
...
let create ~repr ~ml_bundles () = { uuid = Uuid_unix.create (); ml_bundles; repr }
