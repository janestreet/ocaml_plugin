...
let clean t =
  let%bind r1 = Dynloader.clean t.loader in
  let%map r2 =
...
let archive () =
  ...
  && String.equal (Bigstring.to_string bstr) dummy
  then None
  else Some bstr
;;
    ...
    (let str = archive_metadata_binding () in
     ...
     then
       Or_error.error_string
         "This executable does not have an embedded archive, although this is required \
          when using [Ocaml_plugin].  A likely cause is that the build of the binary is \
          missing the step involving [ocaml_embed_compiler]."
...
let () =
  ...
  | Some _ ->
    ...
    (match force archive_metadata with
     ...
     | Ok archive_metadata ->
       Core.Printf.eprintf
         !"archive metadata: %{sexp:Archive_metadata.t}\n%!"
         archive_metadata);
    (match archive () with
...
let save_archive_to destination =
  Deferred.Or_error.try_with ~rest:`Log (fun () ->
    ...
    | Some contents ->
      Writer.with_file_atomic destination ~f:(fun w ->
        Writer.schedule_bigstring w contents;
        Deferred.unit))
;;

type 'a create_arguments =
  (?persistent_archive_dirpath:string -> 'a) Dynloader.create_arguments
...
module Plugin_archive : sig
  ...
  module Info = struct
    ...
    let create () =
      let%bind.E archive_metadata = return (force archive_metadata) in
      let%map.E infos =
      ...
      { infos; build_info; archive_metadata }
    ;;

    let info_file_perm = 0o644

    let save dir =
      let%bind.E t = create () in
      Deferred.Or_error.try_with ~rest:`Log ~extract_exn:true (fun () ->
    ...
    let is_up_to_date t ~dir =
      ...
      | Ok archive_metadata ->
        ...
        if [%compare.equal: Plugin_cache.Digest.t String.Map.t]
             digests
             t.archive_metadata.archive_digests
        then (
          ...
          let%map.E files =
            Deferred.Or_error.try_with ~rest:`Log (fun () -> Sys.readdir dir)
          in
          let files_extracted =
            Set.diff
        ...
        if_ persistent (fun () ->
          ...
          let%bind.E () =
            Monitor.try_with_or_error ~rest:`Log (fun () ->
              Unix.mkdir ~p:() ~perm:0o755 (Filename.dirname lock_filename))
          in
          let%bind.E () = Lock_file_async.Nfs.create lock_filename in
          archive_lock := Archive_lock.Locked lock_filename;
          let%bind.E () = Shell.rm ~r:() ~f:() [ compiler_dir ] in
          Monitor.try_with_or_error ~rest:`Log (fun () ->
            Unix.mkdir ~p:() ~perm:0o755 compiler_dir))
      in
      let destination = compiler_dir ^/ tar_id in
      let%bind.E () = save_archive_to destination in
      let%bind.E () = Tar.extract ~working_dir:compiler_dir destination in
      if_ persistent (fun () -> Info.save compiler_dir)
    in
    if persistent
    then
      Throttle.enqueue extract_throttle (fun () ->
        match%bind Info.load compiler_dir with
        | Error _ -> extract ()
        | Ok info ->
          (match%bind Info.is_up_to_date info ~dir:compiler_dir with
...
let create
  ...
  let%bind.E persistent_archive_dirpath =
    match persistent_archive_dirpath with
    | None -> Deferred.return (Ok None)
    | Some path ->
      let%map.E path = Shell.absolute_pathname path in
  ...
  in
  let%bind.E preprocessor = return preprocessor in
  ...
  let%bind.E loader =
    Dynloader.create
      ...
      ()
  in
  let compiler = { loader; archive_lock } in
...
let with_compiler
  ...
  then return (Or_error.error_s [%sexp "Shutting_down", [%here]])
    ...
    let%bind.E (`this_needs_manual_cleaning_after compiler) =
      create
        ?in_dir
        ?in_dir_perm
        ?include_directories
...
let make_check_plugin_cmd ~check_ocaml_src_files ~load_ocaml_src_files () =
  let execute_plugin_toplevel_switch = "-execute-plugin-toplevel" in
  Command.async
    ~summary:"Check a plugin for compilation errors"
    ~readme:(fun () ->
      String.concat
        [ "This command checks that a plugin compiles.  It either succeeds quietly, or \
           outputs\n\
           compilation errors and fails.\n\n\
           When it is deemed safe to execute the toplevel of a plugin, one can supply \
           the switch\n\
           ["
        ; execute_plugin_toplevel_switch
        ; "] to check for runtime exceptions at toplevel."
        ])
    (let open Command.Let_syntax in
     let%map_open plugin_filenames =
       anon (sequence ("path/to/plugin.ml" %: Filename_unix.arg_type))
     ...
     and is_verbose = flag "-verbose" no_arg ~doc:" Be more verbose" in
     fun () ->
       let open! Deferred.Let_syntax in
       let f compiler =
         let loader = loader compiler in
         let%bind.E plugin_filenames =
           if use_ocamldep
           then (
             ...
             | [] | _ :: _ :: _ ->
               return
                 (Or_error.error
                    "Give only the main file when using option -ocamldep"
                    plugin_filenames
                    [%sexp_of: string list]))
           else return (Ok plugin_filenames)
         in
         if is_verbose
         then Print.printf "checking: %s\n%!" (String.concat ~sep:" " plugin_filenames);
         if execute_plugin_toplevel
         then load_ocaml_src_files loader plugin_filenames >>| Or_error.map ~f:ignore
         else check_ocaml_src_files loader plugin_filenames
       in
       match%map with_compiler ~f () with
       | Ok () ->
