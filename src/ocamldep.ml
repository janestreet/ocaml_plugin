    ...
    | target_file :: ":" :: deps ->
      Or_error.try_with_join (fun () ->
        ...
        then
          ...
          Or_error.error
            "Unexpected target file in ocamldep output"
            (target, line)
            [%sexp_of: string * string])
    | _ -> Or_error.error "Fail to parse ocamldep output" lines [%sexp_of: string list])
;;

let%test_module _ =
  (module struct
    let lines =
      [ "a.cmx : a.cmi c.cmx d.cmx e.cmx"; "a.cmo : a.cmi b.cmo c.cmo"; "a.cmi : f.cmi" ]
    ;;
  ...
  *)
  ...
  let rec visit target =
    ...
    then (
      let circle =
        target
        :: List.rev (List.take_while (Stack.to_list visiting) ~f:(String.( <> ) target))
      in
      return
        (Or_error.error "Circular dependency detected" circle [%sexp_of: string list]))
    else if Queue.mem ~equal:String.equal visit_finish_order target
    then return (Ok ())
    else (
      visit_trace target;
      Stack.push visiting target;
      let%bind.E deps = find_direct_deps ~target in
      let%map.E () =
        Deferred.List.fold (Set.to_list deps) ~init:(Ok ()) ~f:(fun acc dep ->
          let%bind.E () = return acc in
          visit dep)
      in
...
let%test_module _ =
  (module struct
    let test graph ~target =
      let graph =
        String.Map.of_alist_exn
          (List.rev_map graph ~f:(fun (a, deps) -> a, String.Set.of_list deps))
      in
      let visited = String.Hash_set.create () in
      let visit_trace s =
        if Hash_set.mem visited s
        then
          raise_s
            [%sexp
    ...
    let%test_unit _ =
      let graph =
        [ "a", [ "b"; "c"; "d" ]; "b", [ "c" ]; "c", [ "d" ]; "d", [ "e" ]; "e", [] ]
      in
      [%test_eq: string list]
        (test graph ~target:"a" |> ok_exn)
        [ "e"; "d"; "c"; "b"; "a" ]
    ;;

    let%test_unit _ =
      let all =
        List.init 26 ~f:(fun i -> String.make 1 (Char.of_int_exn (Char.to_int 'a' + i)))
      in
      let graph =
