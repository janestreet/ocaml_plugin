  ...
  don't_wait_for
    (Ocaml_plugin.Private.Shell.set_defaults ~verbose:true ~echo:true ();
     ...
     let use_cache =
       ...
       then Some (Ocaml_plugin.Plugin_cache.Config.t_of_sexp (Sexp.of_string argv.(1)))
       else None
     in
     let persistent_archive_dirpath =
       if Array.length argv > 2 then Some argv.(2) else None
     in
     match%bind
       Ocaml_plugin.Compiler.create ?use_cache ?persistent_archive_dirpath ()
     with
     | Error e ->
     ...
     | Ok (`this_needs_manual_cleaning_after compiler) ->
       ...
       let rec loop () =
         ...
         | `Eof ->
           let%bind result = Ocaml_plugin.Compiler.clean compiler in
         ...
         | `Ok input ->
           ...
           let files =
             List.filter_map files ~f:(fun s ->
               let s = String.strip s in
               if String.is_empty s then None else Some s)
           in
           let with_files files =
             match%bind Plugin.load_ocaml_src_files loader files with
             | Error err ->
           ...
           (match files with
            ...
            | [ cmxs ] when String.is_suffix ~suffix:"cmxs" cmxs ->
              ...
              (try Dynlink.loadfile_private cmxs with
               | e ->
                 let str =
                   match e with
            ...
            | [ "dep"; file ] ->
              ...
              (match%bind Ocaml_plugin.Dynloader.find_dependencies loader file with
               | Error err ->
