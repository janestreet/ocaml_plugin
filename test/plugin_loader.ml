...
let run
  ...
  match%bind
    ...
    then (
      ...
      | [ file ] ->
        Ocaml_plugin.Compiler.with_compiler
          ...
          ~f:(fun compiler ->
            ...
            Ocaml_plugin.Dynloader.find_dependencies loader file
            >>=? fun files ->
      ...
      | _ ->
        failwithf
          "When --find-dependencies is specified, only one file should be given"
          ())
    else
...
let groups l =
  List.map
    (String.split ~on:'|' (String.concat l ~sep:" "))
    ~f:(fun s ->
...
let command =
  Command.async
    ~summary:"unit test program for ocaml-plugin"
    (let open Command.Let_syntax in
     let%map_open use_cache =
       ...
       and cache_size =
         flag
           "--cache-size"
           (optional_with_default max_files_default int)
           ~doc:(sprintf " specify size of plugin cache. default %d" max_files_default)
       in
       if cache then Some (use_cache ~max_files:cache_size) else None
     and persistent_archive_dirpath =
       map
         ~f:(fun b -> if b then Some "cache" else None)
         (flag
            "--persistent-archive"
            no_arg
     ...
     and find_dependencies =
       flag "--find-dependencies" no_arg ~doc:" use ocamldep to generate dependencies"
     and files = anon (sequence ("<ocaml-file>" %: Filename_unix.arg_type)) in
     fun () ->
