...
module Flag = struct
  let string tag ~doc fct () =
    Command.Spec.(
      step (fun main strings ->
        List.iter ~f:fct strings;
        main)
      +> flag tag (listed string) ~doc)
  ;;
...
module Flags = struct
  ...
  let error =
    Flag.set
      "-error"
      ~doc:
        (Printf.sprintf " load an ill-typed config to see what happens %s" conf_v1_error)
      load_error
  ;;

  let clean = Flag.set "-clean" ~doc:" clean the builddir after dynamic loading" clean
...
let handle_error load = function
  | Result.Ok m -> load m
  | Result.Error e -> Core.Printf.eprintf "Error:\n%s\n%!" (Error.to_string_hum e)
;;
...
let load_file loader = function
  ...
  | `util files ->
    let res =
      Ocaml_plugin.Dynloader.Side_effect.load_ocaml_src_files
        loader
        (Flags.split_files files)
    in
    res >>| handle_error ignore
;;

let load_default loader =
  let%bind () = load_file loader (`v1 conf_01) in
...
let main () () =
  let k_clean_loader =
    let compilation_directory = if !temp_dir then None else Some builddir in
    let include_directories =
      let directories = List.rev !include_directories in
      if !default && not !embed_mode then def_include @ directories else directories
    in
    ...
    then (
      let build =
        Ocaml_plugin.Compiler.create ~include_directories ~cmx_flags ~cmxs_flags ()
      in
      match%bind build with
      | Error e ->
      ...
      | Ok loader -> (fun () -> Ocaml_plugin.Dynloader.clean loader), loader)
  in
  let dt =
    let%bind k_clean, loader = k_clean_loader in
    let%bind () = if !default then load_default loader else Deferred.return () in
    let%bind () =
...
let command =
  Command.group
    ~summary:"Toy sample program for ocaml_plugin"
    [ "run", run_command
