...
(**
   ...
   This is an low level module, casual user may rather use 'Ocaml_compiler'
...
type 'a create_arguments =
  ?in_dir:string
  (**
     The pathname has to be explicit. If not, the behavior is unspecified.

     This is the location where the ocaml compilation will take place. A fresh and uniq
     directory will be created inside that directory. The default value is
     [Filename.temp_dir_name], so the default behavior is to create a fresh directory
     in the temp_dir_name, like /tmp/ocaml_plugin.1464h45
...
val create : (
  ?initialize_compilation_callback:(
    string -> Config.t option Deferred.Or_error.t
  )
  (**
     ...
     will extract its tar file. The [t] is the dynloader itself.
  *)
  ...
  *)
  ...
  (**
     ...
     with the given camlp4o_opt, as they will be dynlinked.
  *)
...
(**
   Cleaning the files generated by this Loader.t from the begining of his life,
   ...
   it alone and gets collected by the GC at some point. Using a cleaned dynloader will
   result in an error.
*)
...
sig
  (**
     ...
     The field [repr] is used to represent a string in the OCaml syntax
     ...
     The field [univ_constr_repr] is a string in the OCaml syntax to talk
     in a generated code about the ocaml value used as field [univ_constr]
...
module Make : functor (X : Module_type) -> sig
  (**
     Load an ocaml file source file (.ml) pretending that this file is a module
     of a given signature represented by the X.repr module interface.
     If the type does not match, there will be an error during OCaml compilation.
     The file is copied into the compilation directory, and compiled versus
     a generated mli file including this module signature, and then
     is dynlinked with the current executable.
     ...
     (cannot be referenced) from other modules dynamically loaded afterward.
  *)
